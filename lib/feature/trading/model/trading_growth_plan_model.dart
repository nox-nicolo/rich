...
extension GrowthDayStatusX on GrowthDayStatus {
  String get label {
    switch (this) {
      case GrowthDayStatus.pending:   return 'PENDING';
      case GrowthDayStatus.completed: return 'DONE';
      case GrowthDayStatus.missed:    return 'MISSED';
    }
  }
}
...
class TradingGrowthPlan {
  ...
  int    get completedDays => days.where((d) => d.status == GrowthDayStatus.completed).length;
  int    get missedDays    => days.where((d) => d.status == GrowthDayStatus.missed).length;
  ...
  double get currentCapital {
    final marked = days.where((d) =>
        d.status == GrowthDayStatus.completed ||
        d.status == GrowthDayStatus.missed).toList();
  ...
  static TradingGrowthPlan build({
    ...
    for (int i = 1; i <= totalDays; i++) {
      ...
      days.add(GrowthPlanDay(
        ...
        lotSize:      lotSizeForBalance(balance),
      ));
      balance = end;
    }
  ...
  TradingGrowthPlan markDay(int day, GrowthDayStatus status, {double? actualEnd}) {
  ...
  factory TradingGrowthPlan.fromMap(Map<String, dynamic> m) => TradingGrowthPlan(
    id:                 m['id'] as String,
    ...
    days:               (m['days'] as List)
        .map((d) => GrowthPlanDay.fromMap(Map<String, dynamic>.from(d as Map)))
        .toList(),
