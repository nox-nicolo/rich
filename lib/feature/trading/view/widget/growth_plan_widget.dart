    ...
    return Column(
      ...
      children: [
        ...
        if (state.growthPlans.where((p) => !p.isActive).isNotEmpty) ...[
          ...
          ...state.growthPlans.where((p) => !p.isActive).map((p) =>
            _PastTile(plan: p, onDelete: () => vm.deleteGrowthPlan(p.id)),
          ),
        ],
      ],
    );
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _Field(ctrl: startCtrl,  hint: 'Starting capital (USD)',
                  inputType: const TextInputType.numberWithOptions(decimal: true)),
              const SizedBox(height: 10),
              _Field(ctrl: targetCtrl, hint: 'Target capital (USD)',
                  inputType: const TextInputType.numberWithOptions(decimal: true)),
              const SizedBox(height: 10),
              _Field(ctrl: rateCtrl,   hint: 'Daily growth % (e.g. 25)',
                  inputType: TextInputType.number),
              const SizedBox(height: 10),
              _Field(ctrl: daysCtrl,   hint: 'Number of days (e.g. 30)',
                  inputType: TextInputType.number),
              const SizedBox(height: 10),
              _Field(ctrl: slCtrl,    hint: 'Max SL per step % (e.g. 2)',
                  inputType: const TextInputType.numberWithOptions(decimal: true)),
              const SizedBox(height: 16),
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    if (name.isEmpty || start == null || target == null ||
                        rate == null || days == null) {
                      return;
                    }
                    Navigator.pop(ctx);
        ...
        children: [
          Row(
            children: [
              ...
              Text('${plan.dailyGrowthPercent.toStringAsFixed(0)}%/day · SL ${plan.stopLossPercent.toStringAsFixed(0)}%',
          ...
          Row(
            ...
            children: [
              Text('Day ${plan.completedDays}/${plan.totalDays}',
                  style: AppTypography.caption),
              Text('\$${plan.startingCapital.toStringAsFixed(2)} → \$${plan.targetCapital.toStringAsFixed(2)}',
          ...
          Expanded(
            ...
            child: isPending
                ? Row(
                    ...
                    children: [
                      _Btn(label: '✓', color: AppColors.success,
                          onTap: () => vm.markGrowthDay(
                              planId, day.day, GrowthDayStatus.completed)),
                      const SizedBox(width: 4),
                      _Btn(label: '✗', color: AppColors.warning,
                          onTap: () => vm.markGrowthDay(
                              planId, day.day, GrowthDayStatus.missed)),
                    ],
                  )
                : Align(
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
            color: AppColors.warning.withValues(alpha: 0.4), width: 0.5),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: Text('RECALIBRATE — NEW PLAN',
                  style: AppTypography.chip.copyWith(
                      color: AppColors.warning, letterSpacing: 2)),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
            color: AppColors.success.withValues(alpha: 0.4), width: 0.5),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text('START NEW PLAN',
                  style: AppTypography.chip.copyWith(
                      color: AppColors.background, letterSpacing: 2)),
              style: ElevatedButton.styleFrom(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 4),
            Text('Tap NEW PLAN to build your daily goal table',
                style: AppTypography.caption),
          ],
        ),
      ),
...
class _H extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Expanded(
    flex: flex,
    child: Text(text,
        style: AppTypography.chip.copyWith(color: AppColors.textMuted, fontSize: 9)),
  );
}
