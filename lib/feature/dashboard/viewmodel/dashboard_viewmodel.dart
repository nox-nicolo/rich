...
class DashboardViewModel extends StateNotifier<DashboardState> {
  ...
  DashboardViewModel(this._repo, this._ref)
  ...
  void _updateNextAction() {
    ...
    if (!ruleCtx.meditationCompletedToday) {
      ...
      final pending   = workState.todayTasks.where((t) => !t.isCompleted).toList();
      ...
      final active = readingState.allBooks.where((b) => b.readTodayAlready).toList();
      if (state.routineProgress['Reading'] == false) {
        if (active.isNotEmpty) {
          final book = active.first;
          action = 'Reading — ${book.title} (p. ${book.currentPage}/${book.totalPages})';
        } else {
      ...
      if (bettingState.activeBets.isNotEmpty) {
        action = 'Betting — ${bettingState.activeBets.length} active bet(s) to settle';
        route = '/betting';
      ...
      switch (p.feature) {
        ...
        case RichFeature.meditation:
          if (ruleCtx.meditationCompletedToday) {
            return p.copyWith(
              ...
              statusDetail: 'Streak ${meditationState.streak.currentStreak}d — gate open',
            );
          }
          final sessions = meditationState.todaySessions.length;
          return p.copyWith(
            status:       PillarStatus.idle,
            statusDetail: sessions > 0 ? '$sessions session(s) today' : 'Not done — gate closed',
          );

        case RichFeature.work:
          if (locked) return p.copyWith(status: PillarStatus.locked);
          final pending   = workState.todayTasks.where((t) => !t.isCompleted).length;
          final completed = workState.todayTasks.where((t) => t.isCompleted).length;
        ...
        case RichFeature.betting:
          ...
          return p.copyWith(
            ...
            statusDetail: active2 > 0 ? '$active2 active · $plStr today' : 'No active bets',
          );

        case RichFeature.reading:
          if (locked) return p.copyWith(status: PillarStatus.locked);
          final booksRead = readingState.allBooks.where((b) => b.readTodayAlready).length;
          final inProgress = readingState.allBooks.where((b) => b.currentPage > 0 && b.progressPercent < 1.0).length;
          return p.copyWith(
...
final pillarSummariesProvider = Provider<List<PillarSummary>>(
  (ref) {
